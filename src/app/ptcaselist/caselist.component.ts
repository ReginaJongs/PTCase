...
//===新個案(掛號)與初診病歷撰寫===================================================
  newcase() { 
    ...
    this.caseService.newcase(this.uclinic, pid, environment.uempcode)
      .then(response => {
...
//==新建立各類單據=========================================================
  createHDoc(doctype:string, courseid: number, caseid: number){
    ...
    //if (doctype === 'CaseCheckup') patternid = 14;//this.router.navigate(['/form', 3, 13]);
   
    this.caseService.patternadopt('Physio', doctype)
    .then(response => {
      patternid = response[0].doc_id; //alert(doctype +'採用表單:'+ JSON.stringify(response[0].doc_id));
