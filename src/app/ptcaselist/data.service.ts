...
export class DataService {
  ...
  patternadopt(projcode: string, doccode: string): Promise<any> {
    const url = "/api/patternadopt" //採用樣張的docid
    const body = { projcode, doccode}; 
    let options = { headers: new HttpHeaders({'content-type': 'application/json'})};
    return lastValueFrom(this.http.post(url, body, options));
  }
  ...
  createHDoc(patternid:number, courseid:number, caseid:number): Promise<any> {
    ...
    const builderid = environment.uempid;
    const body = { patternid, courseid, caseid, builderid}; 
    let options = { headers: new HttpHeaders({'content-type': 'application/json'})};
