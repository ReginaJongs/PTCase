...
export class CaseListComponent implements OnInit {
  ...
  pageSize = 5; currentPage = 0;//for pagination
...
//===新個案(掛號)與初診病歷撰寫===================================================
  ...
  applyFilter(event: Event) {
    ...
    if (this.dataSource.filteredData.length > 0) {
      this.highlightedElement = this.dataSource.filteredData[0];
    } else {
  ...
  onRowClick(row: any) {
    //alert('點擊的資料列：'+ row);  alert('科別：'+ row.casetype); alert('病人：'+ row.pid);
    this.pidquery = row.pid;
